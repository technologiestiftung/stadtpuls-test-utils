...
git clone --no-checkout https://github.com/technologiestiftung/stadtpuls-supabase.git supabase &&
  ...
  git checkout v4.0.0
# degit the folder so we don't get problems with git trying to add/commit this
rm -rf .git

# copy local seed script into docker init folder
cp ../seeds/globals/*.sql supabase-docker-compose/dockerfiles/postgres/docker-entrypoint-initdb.d/
